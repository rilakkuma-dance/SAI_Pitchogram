...
class PhonemeAnalyzer:
    ...
    def __init__(self):
        # Common phoneme mappings for better alignment
        self.phoneme_variants = {
            'ɕ': ['ʃ', 'x', 's'],
            'ɛ': ['e', 'ɪ', 'ə'],
            '5': ['˥', '¹', '1'],  # tone markers
            'ʃ': ['ɕ', 's'],
    ...
    def normalize_phoneme(self, phoneme):
        """Normalize phoneme for better matching"""
        # Remove tone markers and diacritics for core comparison
        core = phoneme.lower().strip()
        # Remove common tone markers
        tone_markers = ['˥', '˦', '˧', '˨', '˩', '¹', '²', '³', '⁴', '⁵', '1', '2', '3', '4', '5']
    ...
    def phoneme_similarity(self, p1, p2):
        ...
        n1, n2 = self.normalize_phoneme(p1), self.normalize_phoneme(p2)
        
        if n1 == n2:
            return 0.9  # High similarity if core sounds match
        
        # Check phoneme variants
        variants1 = self.phoneme_variants.get(n1, [n1])
        variants2 = self.phoneme_variants.get(n2, [n2])
        
        for v1 in variants1:
            if v1 in variants2 or v1 == n2:
                return 0.7
        ...
        # Character similarity for unknown phonemes
    ...
    def analyze_pronunciation(self, detected: str, target: str):
        ...
        aligned_detected, aligned_target = self.align_phonemes(detected, target)
...
class SimpleWav2Vec2Handler:
    ...
    def __init__(self, model_name="facebook/wav2vec2-xlsr-53-espeak-cv-ft", sample_rate=16000):
    ...
    def load_model(self):
        try:
            ...
            # Manual component loading to avoid the boolean tokenizer issue
    ...
    def parse_phonemes(self, phoneme_string):
        """Parse continuous phoneme string into individual phonemes"""
        if not phoneme_string:
            return []
        
        import re
        
        # Pattern to match common phonemes with optional tone markers
        # This covers your Chinese phonemes: ɕ, i, ɛ, and tone numbers
        patterns = [
            r'ɕiɛ[1-5]',   # Complete syllables like ɕiɛ5
            r'ɕi[1-5]',    # Syllables like ɕi5  
            r'iɛ[1-5]',    # Vowel combinations with tone
            r'[ɕɔɛʃstdn][iɛaouəy]*[1-5]?',  # Consonant + vowel(s) + optional tone
            r'[iɛaouəy]+[1-5]?',  # Vowel sequences with optional tone
            r'[1-5]',      # Standalone tone markers
            r'[ɕɔɛʃstdn]' # Standalone consonants
        ]
        
        # Combine all patterns
        full_pattern = '|'.join(patterns)
        phonemes = re.findall(full_pattern, phoneme_string)
        
        # If regex didn't work well, try simple splitting
        if not phonemes or len(''.join(phonemes)) < len(phoneme_string) * 0.7:
            # Fallback: split by likely boundaries
            phonemes = []
            i = 0
            while i < len(phoneme_string):
                # Look for tone numbers
                if i < len(phoneme_string) - 1 and phoneme_string[i+1] in '12345':
                    phonemes.append(phoneme_string[i:i+2])
                    i += 2
                else:
                    phonemes.append(phoneme_string[i])
                    i += 1
        
        # Clean up empty phonemes
        phonemes = [p for p in phonemes if p.strip()]
        print(f"Parsed '{phoneme_string}' into: {phonemes}")
        return phonemes
        ...
        try:
            ...
            try:
                ...
                if cleaned_transcription and len(cleaned_transcription.strip()) > 0:
                    print(f"Raw transcription: '{cleaned_transcription}'")
                    
                    # Parse the detected phonemes
                    parsed_detected = self.parse_phonemes(cleaned_transcription)
                    parsed_target = self.parse_phonemes("ɕ iɛ5 ɕ iɛ5")  # Your target
                    
                    # Join them back with spaces for comparison
                    spaced_detected = ' '.join(parsed_detected)
                    spaced_target = ' '.join(parsed_target)
                    
                    print(f"Parsed detected: {spaced_detected}")
                    print(f"Parsed target: {spaced_target}")
                    
                    self.result = spaced_detected  # Use the parsed version
                else:
                    self.result = "no_audio"
...
class SAIVisualizationWithWav2Vec2:
    ...
    # Enhanced phoneme similarity analysis with more detailed categorization
    def analyze_pronunciation(self, detected, target):
        """Analyze pronunciation and return detailed feedback with enhanced scoring"""
        if not detected and not target:
            return [], 0.0
        ...
        for det, tgt in zip(aligned_detected, aligned_target):
            if det is None and tgt is not None:
                ...
                # Enhanced status classification with more granular levels
                if similarity >= 0.95:
                    status = 'perfect'
                elif similarity >= 0.8:
                    status = 'correct'
        ...
        try:
            ...
            try:
                ...
                if detected and detected != "no_audio":
                    ...
                    # Create the detailed phoneme feedback display
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        default_audio = os.path.join(script_dir, 'reference', 'chinese_ma.mp3')
