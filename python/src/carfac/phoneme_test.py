...
import modules.phoneme_handler as phoneme_handler
...
class DualSAIWithRecording:
    def __init__(self, audio_file_path=None, chunk_size=1024, sample_rate=16000, sai_width=200,
        ...
        # Initialize phoneme handler
        self.phoneme_handler = phoneme_handler.PhonemeHandler()
    ...
    def _load_audio_file(self):
        ...
        self.file_phonemes = self.phoneme_handler.transcribe_audio_from_tensor(torch.Tensor(self.audio_data))
        self.whisper_file.add_transcription_line(" ".join(self.file_phonemes))

        self.text_similarity_calculator = TextSimilarityCalculator(self.file_transcription)
        self.phoneme_similarity_calculator = TextSimilarityCalculator(" ".join(self.file_phonemes))
    ...
    def _process_recorded_audio(self):
        ...
        try:
            ...
            phoneme_transcription = self.phoneme_handler.transcribe_audio_from_tensor(torch.Tensor(recorded_array))
            ...
            if transcription and len(transcription.strip()) > 0:
                ...
                print(f"Recorded phonemes: {phoneme_transcription}")
                self.whisper_realtime.add_transcription_line(" ".join(phoneme_transcription))
                # text_distance = self.text_similarity_calculator.compare_texts(transcription)
                phoneme_distance = self.phoneme_similarity_calculator.compare_texts(" ".join(phoneme_transcription))

                if phoneme_distance == 0:
                    self.transcription_realtime.set_color('lime')
                    self.score_display.set_text("PERFECT MATCH!")
                    self.score_display.set_color('lime')
                    print("Perfect match!")
                elif phoneme_distance <= 1: 
                    self.transcription_realtime.set_color('lime')
                    self.score_display.set_text("EXCELLENT MATCH!")
                    self.score_display.set_color('lime')
                    print("Excellent match!")
                elif phoneme_distance <= 3:
                    self.transcription_realtime.set_color('orange')
                    self.score_display.set_text(f"GOOD MATCH!")
                    self.score_display.set_color('orange')
                    print(f"Good match (distance: {phoneme_distance})")
                else:
                    self.transcription_realtime.set_color('red')
                    self.score_display.set_text(f"NEEDS PRACTICE")
                    self.score_display.set_color('red')
                    print(f"Poor match (distance: {phoneme_distance})")
            else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        default_audio = os.path.join(script_dir, 'reference', 'thai_thankyou.mp3')
