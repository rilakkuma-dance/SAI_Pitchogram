...
class DualSAIProcessor:
    ...
    def _setup_dual_visualization(self):
        """Setup dual screen visualization"""
        self.fig = plt.figure(figsize=(20, 10))
        
        # Create two subplots side by side
        self.ax_realtime = plt.subplot(1, 2, 1)
        self.ax_file = plt.subplot(1, 2, 2)
    ...
    def get_next_file_chunk(self):
        ...
        if not self.audio_data is not None or self.current_position >= self.total_samples:
            return None
    ...
    def update_visualization(self, frame):
        ...
        try:
            ...
            if self.audio_data is not None:
                # Process file chunk
                chunk = self.get_next_file_chunk()
                if chunk is not None:
                    ...
                    pitch_frame = self.vis_file.run_frame(sai_output)
            ...
            if not transcription_file:
                transcription_file = "Processing file..." if self.audio_data is not None else "No audio file loaded"
            self.transcription_file.set_text(transcription_file)
            
            # Update progress for file
            if self.audio_data is not None:
                current_time = self.chunks_processed * self.chunk_size / self.sample_rate
                progress_percent = (current_time / self.duration) * 100 if self.duration > 0 else 0
    ...
    def start(self):
        ...
        # Print status
        if self.audio_data is not None:
            print(f"File processing ready: {self.duration:.2f} seconds")
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        processor = DualSAIProcessor(debug=True)
        try:
            processor.start()
        except KeyboardInterrupt:
            print("\nShutting down...")
