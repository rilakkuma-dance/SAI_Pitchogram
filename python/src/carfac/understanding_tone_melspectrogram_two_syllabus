...
class ToneSpectrogramQuiz:
    ...
    def _find_audio_folder(self):
        ...
        potential_path = script_dir / 'mandarin_audio_two_syllabus'
        if potential_path.exists(): return potential_path
        potential_path = script_dir.parent / 'mandarin_audio_two_syllabus'
        if potential_path.exists(): return potential_path

        print("⚠️ Could not auto-detect 'mandarin_mandarin_audio_two_syllabus' folder.")
        root = tk.Tk()
        root.withdraw() 
        folder_selected = filedialog.askdirectory(title="Select the 'mandarin_audio_two_syllabus' folder")
        root.destroy()
