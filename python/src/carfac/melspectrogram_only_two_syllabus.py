...
ref = Path("carfac/mandarin_audio_two_syllabus")
...
class SpectrogramProcessor:
    """Mel spectrogram processor using librosa - UPDATED TO MATCH QUIZ SETTINGS"""
    def __init__(self, sample_rate=16000, n_fft=512, hop_length=128, n_mels=128):
        ...
        # --- UPDATED SETTINGS FROM SCRIPT 2 ---
        self.fmin = 50
        self.fmax = 4000
    ...
    def process_chunk(self, audio_chunk):
        ...
        # We apply fmin and fmax here to zoom in on the voice range
        melspec = librosa.feature.melspectrogram(
            ...
            n_mels=self.n_mels,
            center=True,
            fmin=self.fmin,  # <--- Focus on 50Hz
            fmax=self.fmax   # <--- Cut off above 4000Hz (Focus on voice)
        )
...
class PracticeSet:
    ...
    def __init__(self, audio_base_path="mandarin_audio_two_syllabus"):
        self.all_items = []
        self.audio_base_path = Path(audio_base_path)

        # Removed Sentences, only Words remaining
        words = [
            {"id": 1,  "chinese": "医生", "pinyin": "yīshēng",   "tone": "11", "audio": "01_医生_11.wav", "type": "word"},
    ...
    def generate_new_set(self):
        ...
        num_items = min(3, len(self.all_items))
        ...
        for i, item in enumerate(self.current_set, 1):
            print(f"{i}. [{item['type'].lower()}] {item['chinese']} ({item['pinyin']})")
...
class SimpleAudioVisualizerWithSAI:
    ...
    def __init__(self, chunk_size=512, sample_rate=16000, sai_width=400, save_dir="recordings", audio_ref_dir="carfac/mandarin_audio_two_syllabus"):
...
if __name__ == "__main__":
    visualizer = SimpleAudioVisualizerWithSAI(
        ...
        sai_width=400,
        save_dir="recordings",
        audio_ref_dir="carfac/mandarin_audio_two_syllabus" 
    )
